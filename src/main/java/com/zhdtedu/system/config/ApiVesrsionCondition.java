...
public class ApiVesrsionCondition implements RequestCondition<ApiVesrsionCondition> {
    ...
    //验证header中是否有version
    private final static Pattern VERSION_PREFIX_PATTERN_HEADER = Pattern.compile("v(\\d+)");
    ...
    public ApiVesrsionCondition getMatchingCondition(HttpServletRequest request) {
        ...
        boolean flag = m.find();
        if(!flag){
            uri = request.getHeader("version");
            m = VERSION_PREFIX_PATTERN_HEADER.matcher(uri);
            flag = m.find();
        }
        if(flag) {
